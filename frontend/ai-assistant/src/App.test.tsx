...
describe('AI Assistant island', () => {
  ...
  test('shows course-aware suggestions only after selecting a course first', async () => {
    ...
    expect(await screen.findByText(/What kind of quiz would you like to create/i)).toBeVisible();
    expect(await screen.findByLabelText(/Describe the quiz you want to create/i)).toBeDisabled();
  ...
  test('keeps advanced controls and readiness on the same Quiz Plan state', async () => {
    ...
    await waitFor(() => expect(course).toHaveValue('7'));
  ...
  test('sends an optional teacher-selected title when generating a draft', async () => {
    const detail = conversation({
      courseId: 7,
      status: 'ready_to_generate',
      plan: {
    ...
    const title = await screen.findByRole('textbox', { name: /Draft title/i });
    await waitFor(() => expect(title).toBeEnabled());
    await user.type(title, 'Teacher-selected Data Structures Quiz');
    await user.click(screen.getByRole('button', { name: 'Generate Draft' }));

    await waitFor(() => expect(api.generateAiConversationDraft).toHaveBeenCalled());
    expect(vi.mocked(api.generateAiConversationDraft).mock.calls[0][2])
  ...
  test('saves an untouched AI-generated quiz without requiring a manual question', async () => {
    const generatedDraft = {
      id: 9,
      quizId: 77,
      title: 'Generated Loops Quiz',
    ...
    expect(screen.getByText('Ready to save as draft')).toBeVisible();
    await user.click(screen.getByRole('button', { name: 'Open in Quizzes' }));
    expect(onNavigate).toHaveBeenCalledWith('#/quizzes');
    expect(saveButton).toBeEnabled();
    await user.click(saveButton);

    await waitFor(() => expect(api.saveAiConversationDraft).toHaveBeenCalledWith(41, generatedDraft));
    expect(saveButton).toBeDisabled();
  });

  test('explains an empty revision request and submits it after the teacher adds an instruction', async () => {
    const detail = conversation({
      title: 'Loops quiz',
      status: 'review_required',
      courseId: 7,
    ...
    expect(screen.getByText('Describe what you want to change first.')).toBeVisible();
    expect(api.reviseAiConversationDraft).not.toHaveBeenCalled();

    const composer = screen.getByLabelText('Describe a revision to the quiz draft');
    await user.type(composer, 'Make the first question harder.');
    expect(screen.queryByText('Describe what you want to change first.')).not.toBeInTheDocument();
    await user.click(revisionButton);
    await waitFor(() => expect(api.reviseAiConversationDraft).toHaveBeenCalledWith(
      41,
      'Make the first question harder.'
    ));
  });

  test('keeps contextual suggestions clickable after generation has completed', async () => {
    const detail = conversation({
      status: 'ready_to_generate',
      courseId: 7,
      generation: {
        status: 'completed',
        stage: 'opening_review_workspace',
        canCancel: false
      },
      plan: {
        ...DEFAULT_PLAN,
    ...
    const suggestion = await screen.findByRole('button', { name: 'Keep 2 MCQ' });
    expect(suggestion).toBeEnabled();
    await user.click(suggestion);
    await waitFor(() => expect(api.sendAiConversationMessage).toHaveBeenCalledWith(
      41,
      'Keep the Loops quiz at 2 multiple-choice questions.'
    ));
  });
